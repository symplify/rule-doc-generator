...
final class DirectoryToMarkdownPrinter
{
    ...
    public function __construct(RuleDefinitionClassesFinder $classByTypeFinder, SymfonyStyle $symfonyStyle, \Symplify\RuleDocGenerator\RuleDefinitionsResolver $ruleDefinitionsResolver, RuleDefinitionsPrinter $ruleDefinitionsPrinter)
    {
    ...
    private function filterOutSkippedTypes(array $ruleClassWithFilePaths, array $skipTypes) : array
    {
        ...
        $ruleClassWithFilePaths = \array_filter($ruleClassWithFilePaths, static function (RuleClassWithFilePath $ruleClassWithFilePath) : bool {
            return $ruleClassWithFilePath->isDeprecated() === \false;
        });
        if ($skipTypes === []) {
