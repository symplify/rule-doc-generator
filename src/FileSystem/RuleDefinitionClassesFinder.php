...
use RuleDocGenerator202408\Nette\Loaders\RobotLoader;
use ReflectionClass;
...
final class RuleDefinitionClassesFinder
{
    ...
    public function findInDirectories(array $directories) : array
    {
        ...
        foreach ($robotLoader->getIndexedClasses() as $class => $filePath) {
            if (!\is_a($class, self::DOCUMENTED_RULE_INTERFACE, \true)) {
                continue;
            }
    ...
    {
        $reflectionClass = new ReflectionClass($class);
        return \strpos((string) $reflectionClass->getDocComment(), '@deprecated') !== \false;
    }
}
