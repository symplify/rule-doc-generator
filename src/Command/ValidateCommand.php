...
final class ValidateCommand extends Command
{
    ...
    /**
     * @readonly
     * @var \Symplify\RuleDocGenerator\FileSystem\RuleDefinitionClassesFinder
     */
    private $ruleDefinitionClassesFinder;
    public function __construct(SymfonyStyle $symfonyStyle, RuleDefinitionClassesFinder $ruleDefinitionClassesFinder)
    {
        $this->symfonyStyle = $symfonyStyle;
        $this->ruleDefinitionClassesFinder = $ruleDefinitionClassesFinder;
        parent::__construct();
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        foreach ($classesByFilePaths as $ruleClass) {
            ...
            if (\strlen($ruleDefinition->getDescription()) < 10) {
                $this->symfonyStyle->error(\sprintf('Rule definition "%s" of "%s" is too short. Make it at least 10 chars', $ruleDefinition->getDescription(), $ruleDefinition->getRuleClass()));
                $isValid = \false;
            }
            if (\count($ruleDefinition->getCodeSamples()) < 1) {
                $this->symfonyStyle->error(\sprintf('Rule "%s" does not have any code samples. Ad at least one so documentation is clear', $ruleDefinition->getRuleClass()));
                $isValid = \false;
            }
        }
        if ($isValid === \false) {
